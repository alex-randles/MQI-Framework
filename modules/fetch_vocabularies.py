...
class FetchVocabularies:
    ...
    def http_retrieval(self, url):
        ...
        # redudancy - name should be shared
        filename = self.hash_filename(url)
        file_location = self.cache_directory + filename
    ...
    def get_unique_namespaces(self):
        ...
        query = """
                     ...
                     { 
                        ...
                        && !(STRSTARTS(STR(?o), STR(rr:))) && !(STRSTARTS(STR(?o), STR(rdfs:))) && 
    ...
    def retrieve_ontology(self, url):
        for func in [self.rdflib_retrieval, self.http_retrieval]:  # change list order to change execution order.
            try:
                len = func(url)
                return len
            except Exception as err:
...
if __name__ == "__main__":
    FetchVocabularies("/home/alex/Desktop/testing_mapping.ttl")
